...
public Action:Command_ToggleFriendly(client, args){
	...
	if(!HasAccess(client)){
		ReplyToCommand(client, "%s 你无权限使用此命令", CHAT_PREFIX);
		return Plugin_Handled;
	}
	
	if(!IsInSpawn[client]){
		ReplyToCommand(client, "%s 你应该在出生室使用此命令", CHAT_PREFIX);
		return Plugin_Handled;
	}
	
	new time = GetTime() -cmd_delay[client];
	if(time < DELAY){
		ReplyToCommand(client, "%s 你需要等待 %d 秒", CHAT_PREFIX, DELAY -time);
		return Plugin_Handled;
	}
...
SetClientFriendly(client, bool:set, bool:broadcast=true, bool:force=false){ //force is used only when the plugin itself gets unloaded/reloaded
	...
	if(set){
		...
		if(broadcast) PrintToChat(client, "%s 友善模式已启用", CHAT_PREFIX);
		...
		IsFriendly[client] = false;
		if(broadcast) PrintToChat(client, force ? "%s 你被迫退出友善模式" : "%s 友善模式已禁用", CHAT_PREFIX);
...
public Native_CheckIfFriendly(Handle:plugin, numParams){
	...
	if(client < 1 || client > MaxClients){
		ThrowNativeError(SP_ERROR_PARAM, "无效的客户端或客户端索引 %i", client);
		return false;
	}
	
	if(!IsClientInGame(client)){
		ThrowNativeError(SP_ERROR_PARAM, "客户端 %i 不在游戏中", client);
		return false;
	}
...
public Action:RTD_CanRollDice(client){
	...
	if(IsFriendly[client]){
		PrintToChat(client, "%s 友善模式下无法使用 RTD", CHAT_PREFIX);
		return Plugin_Handled;
	}
