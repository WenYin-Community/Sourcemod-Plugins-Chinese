...
public void OnPluginStart() {
	...
	ItemDefine("气爆", "airblast", "所有喷火器的气爆（m2）判定箱还原至丛林炼狱更新前", ITEM_FL_PICKABLE);
	ItemDefine("空袭火箭筒", "airstrike", "不再有额外的爆炸半径惩罚");
	ItemDefine("大使左轮手枪", "ambassador", "所有距离下均能造成完整的爆头伤害（102）");
	ItemDefine("原子能球棒", "atomizer", "没有拿着也可以三重跳，每次三段跳会受到10点伤害", ITEM_FL_PICKABLE);
	ItemDefine("火焰战斧", "axtinguish", "始终对燃烧目标造成195点暴击伤害", ITEM_FL_PICKABLE);
	ItemDefine("极限降落伞", "basejump", "可以重新部署降落伞，更好的空中控制，使着火的玩家会由气流上升");
	ItemDefine("娃娃脸的冲击波", "babyface", "受到伤害时不损失速度加成，二段跳时仅减少25%速度条");
	ItemDefine("乞丐火箭筒", "beggars", "无半径惩罚，发射失败不消耗弹药", ITEM_FL_PICKABLE);
	ItemDefine("原子能饮料", "bonk", "效果消失后不再减速");
	...
	SetMenuTitle(menu_main, "旧版武器恢复");
	SetMenuPagination(menu_main, MENU_NO_PAGINATION);
	SetMenuExitButton(menu_main, true);
	AddMenuItem(menu_main, "info", "显示有关每个恢复的信息");
	AddMenuItem(menu_main, "pick", "选择要恢复使用的项目");
